# 3D 큐브 — 플레이스토어 등록 정보 (14개 언어)

앱: 3D 큐브 / 3D Cube · 패키지 `com.ymlee.cube`
...
- 데이터 보안 신고에서 **“사진 또는 동영상” 수집에는 체크하지 않는다.**
  카메라 권한은 쓰지만 이미지를 저장·전송하지 않기 때문이다
  (근거는 개인정보처리방침 2번).
...
■ Also
· The net view shows all six faces at once — no turning it round to check the back.
...
■ そのほか
・展開図が6面を一度に表示します。裏を見るために回す必要はありません。
・指の操作と手順ボタンの両方が使えます。
・ダークとライトを選べます。
・14言語に対応しています。
・インターネット接続は不要です。
...
■ Además
· La vista desplegada muestra las seis caras a la vez: no hay que darle la vuelta.
...
■ Et aussi
· La vue dépliée montre les six faces d'un coup : inutile de retourner le cube.
...
■ Außerdem
· Die Netzansicht zeigt alle sechs Seiten gleichzeitig — kein Umdrehen nötig.
...
■ Além disso
· A vista planificada mostra as seis faces de uma vez — sem precisar virar o cubo.
...
■ Кроме того
· Развёртка показывает все шесть граней сразу — переворачивать не нужно.
...
■ Ngoài ra
· Hình khai triển hiện đủ sáu mặt cùng lúc, khỏi xoay ra sau để xem.
...
■ Selain itu
· Tampilan jaring memperlihatkan keenam sisi sekaligus — tak perlu diputar.
...
■ นอกจากนี้
· แผนภาพคลี่แสดงครบหกด้านพร้อมกัน ไม่ต้องหมุนไปดูด้านหลัง
...
`legal/cube3d/assets/` 에 두었다. 앱 저장소가 비공개가 되어도 여기 남는다.
(원본은 `C:\workAndroid\3Dcube\Build\store\_assets\` 에도 있다.)

| 파일 | 규격 | 콘솔 항목 |
|---|---|---|
| `icon-512.png` | 512 × 512, 알파 없음, 394KB | 앱 아이콘 |
| `feature-graphic-1024x500.png` | 1024 × 500, 알파 없음, 283KB | 그래픽 이미지 |

- 아이콘은 **모서리를 비우지 않았다.** 구글이 자체로 둥글게 깎기 때문에,
  이미 둥근 이미지를 올리면 둥근 모양 안에 또 둥근 모양이 생긴다.
  앱 안에 박히는 런처 아이콘(`Assets/Art/AppIcon.png`)은 둥근 원본 그대로다.
- 그래픽 이미지에는 글자를 앱 이름 하나만 넣었다. 문장을 넣으면 언어마다
  따로 만들어야 하는데, 그림만으로도 무슨 앱인지 보이게 했다.
  14개 언어에 같은 파일을 쓰면 된다.

## 스크린샷 (14개 언어 준비 완료)

모두 **1080 × 2160** 으로 잘라 두었다.
플레이스토어 휴대전화 스크린샷은 긴 변이 짧은 변의 2배를 넘으면 반려된다.
갤럭시 S20 울트라 원본은 1080 × 2400(2.22배), A16 은 1080 × 2340(2.17배)이라
둘 다 상태 표시줄과 내비게이션 바를 잘라 정확히 2:1 로 맞췄다.

파일 위치 — `C:\workAndroid\3Dcube\Build\store\<언어코드>\`

| 파일 | 화면 | 찍은 기기 |
|---|---|---|
| `1_home.png` | 첫 화면 — 큐브 크기 고르기와 메뉴 | S20 울트라 |
| `2_practice.png` | 연습 — 섞인 큐브와 힌트가 뜬 상태 | S20 울트라 |
| `3_learn.png` | 배우기 — 1단계 흰 십자 | S20 울트라 |
| `4_skin.png` | 큐브 스킨 고르기 | S20 울트라 |
| `5_camera.png` | 실물 큐브 촬영 — 색 기준과 인식 격자 | S20 울트라 |
| `6_character.png` | 캐릭터 그림 스킨을 입힌 큐브 | A16 |

언어코드 폴더: `ko en ja zhCN zhTW zhHK es fr de pt ru vi id th`

콘솔에서 언어를 추가할 때마다 그 언어 폴더의 여섯 장을 올린다.
첫 장은 `2_practice.png`(힌트가 보인다)나 `5_camera.png`(실물 큐브 촬영)를 권한다.
이 앱을 고르는 이유가 그 둘이기 때문이다.

## 앱 이름

폰 언어를 따라 런처 이름이 바뀐다. 앱 안에서 고른 언어와는 별개다.

| 폰 언어 | 이름 |
|---|---|
| 기본값(그 밖의 모든 언어) | 3D Cube |
| 한국어 | 3D 큐브 |
| 日本語 | 3Dキューブ |
| 中文(简体·繁體·香港) | 3D魔方 |
| Español · Português | Cubo 3D |
| Français | Cube 3D |
| Deutsch | 3D-Würfel |
| Русский | 3D-кубик |
| Tiếng Việt | Khối 3D |
| Bahasa Indonesia | Kubus 3D |
| ไทย | ลูกบาศก์ 3D |

`aapt dump badging` 으로 빌드 결과에서 확인한 값이다.
...
따로 두지 않으므로 스토어 설명란이 그 자리다. 원문은 앱 소스 저장소의 `ThirdParty/TablerIcons/LICENSE.txt`에 있다.
